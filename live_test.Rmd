...
``` 
```{r convert_to_numeric_testing} 
# Convert columns 8 to end minus the classe column to numeric 
for(i in c(8:ncol(pmlTestingSet)-1)){ 
  pmlTestingSet[,i]=as.numeric(as.character((pmlTestingSet[,i])))
} 

pmlTesting_dataSet <- colnames(pmlTestingSet[colSums(is.na(pmlTestingSet)) == 0])[-(1:7)] 
pmlTesting_ModelDataSet <- pmlTestingSet[pmlTesting_dataSet] 
colnames(pmlTesting_ModelDataSet) 
``` 
